# Brief and client planning


Teams of Cambridge second year computer science students work every year to
deliver a diverse set of projects. Each project is born from a design brief: an
external client proposes a project title and a one-paragraph project brief,
meets with our students four times (or more), and our students as a team deliver
a technical solution.

!!! warning

    All content on this site has draft status, subject to confirmation by
    both group project coordinators and clients. There is no
    guarantee that these projects will be offered to students, either in the
    form described here, or at all.

## Group Project Design Briefs for 2027 (work in progress)

We are currently collecting design briefs for the 2025/2026 group project runs (expecting 131 students, 22 teams). If you are
interested to participate as a client, read ["What makes a good project?"](design_a_good_project.md).
To put forward your project idea, get in touch with [Alan
Blackwell](https://www.cl.cam.ac.uk/~afb21/) or [Tobias Grosser](https://www.grosser.science).

### Accepted design briefs (for 2027)

1. [Maths Recommender](Maths_Recommender)
2. [A Declarative MLIR Dialect for the AHB Protocol](A Declarative MLIR Dialect for the AHB Protocol)

### Design Brief Candidates  (for 2027)

- no candidates yet

### Potential Clients (for 2027)

- Zanado - Quantum Compiler
- Vianca Shah - AI for Medical Tools
- Mosaic - CIRCT EDA Tools
- [IMC](IMC) - Contact Campus events
- Chengyu Zhang - Testing of ITPs

### Design briefs suggested to several potential clients

1.  [The Carbon Eye](The_Carbon_Eye)
2.  [ResponsibleAncestry.com](ResponsibleAncestry.com)
3.  [Guitar tab2hand](Guitar_tab2hand)
4.  [Who Pays for Roads](Who_Pays_for_Roads)?

### Candidates considered in 2026 that may be developed

- [Behind the Eyes](Behind_the_Eyes)
- [Who Pays for Roads](Who_Pays_for_Roads)?
- [Software Inconsistency
  Resolver](Software_Inconsistency_Resolver)
- [Self-Driving Upgrade](Self-Driving-Upgrade)
- [ARM](ARM)
- [Frontier](Frontier)
- Jack Ashby, Zoology museum
- Chris Pointon, Green Economy Infrastructure
- Simon Pulman-Jones, Evinova / Astra Zeneca
- Leontien Talboom, University Library
- Bruno Cardoso Lopes, Meta
- [Sustainable Electronic
    Recycling](Sustainable_Electronic_Recycling)
- [Semantic Refactoring](Semantic_Refactoring)

### Design briefs considered in 2024 that might be developed

1.  [Dignified Distributed Work](Dignified_Distributed_Work)
2.  [The Big Chill](The_Big_Chill)
3.  [Future Health and Fitness](Future_Health_and_Fitness)
4.  [AfroInsight](AfroInsight)
5.  [Leadership Transition
    Simulator](Leadership_Transition_Simulator)
6.  [Environmental Value Added](Environmental_Value_Added)
7.  [Just Maps](Just_Maps)
8.  [Non-WEIRD Data Science](Non-WEIRD_Data_Science)
9.  [Living Salad Bar](Living_Salad_Bar)

### Potential clients for 2025

Final [2025 list](2025)

- [Alder Hey Children's
  Hospital](Alder_Hey_Children's_Hospital)
- [Embecosm](Embecosm)
- [SEAT](SEAT)
- [Anthony Harris](Anthony_Harris)
- [Rivos Systems](Rivos_Systems)
- [AMD](AMD)
- [Raspberry Pi Foundation](Raspberry_Pi_Foundation)
- Katy Jordan, EdTech Hub
- Adham Ashton-Butt, [British Trust for
  Ornithology](British_Trust_for_Ornithology)
- Adam Devenish, RSPB
- Maximilian Ge
- [Matthew Postgate](Matthew_Postgate)

### Incubator and network contacts

- [Centre for Global Equality](Centre_for_Global_Equality)
- [Barn4](Barn4)
- [Ideaspace](Ideaspace)
- [Emma Salgard Cunha](Emma_Salgard_Cunha) - humanities at
  Cambridge Enterprise
- [Nicola Buckley](Nicola_Buckley) - policy fellows at CSaP

### Other client discussions (last updated 2023)

- [SMARTRI Indonesia](SMARTRI_Indonesia)
- [Mark Gotham](Mark_Gotham)
- [Daniel Hall](Daniel_Hall)
- [London Stock Exchange Group](London_Stock_Exchange_Group)
- Neil Walker
- [Milk and More](Milk_and_More)
- [Jane Street](Jane_Street)
- [Fair Finance](Fair_Finance)
- [The Fusion Works](The_Fusion_Works)
- [Frontier](Frontier)
- [Amazon](Amazon)
- [Dovetailed](Dovetailed)
- [JP Morgan](JP_Morgan)
- [Centre for Policy Futures](Centre_for_Policy_Futures)
- [Gardin](Gardin)
- [ICCCAD](ICCCAD)
- [Antobot](Antobot)
- [Morgan Stanley](Morgan_Stanley)
- [Curriculum for Life](Curriculum_for_Life)
- [RSPB](RSPB)
- [BigPay](BigPay)
- [British Trust for
  Ornithology](British_Trust_for_Ornithology)
- [University Information
  Services](University_Information_Services)

### Development notes carried forward from 2021

- [Frontier](Frontier) - second project
- [NIAB](NIAB) - second project
- [Informetis](Informetis) - topic discussed
- [Giving Voice to Digital
  Democracies](Giving_Voice_to_Digital_Democracies) - topic
  suggested, no response
- [ARM](ARM) - contact identified, but no topic

### Other clients from 2021

- [MathWorks](MathWorks)

- [Boeing](Boeing)
- [TechWolf](TechWolf)
- [DX Analytics](DX_Analytics)
- [Umbrella Analytics](Umbrella_Analytics)
- [Lyzeum Ltd](Lyzeum_Ltd)
- [Broadcom](Broadcom) - all contact emails now bounce

## [Previous ideas that have not been used](Previous_ideas_that_have_not_been_used)

- [Calendar Dialogue](Calendar_Dialogue) -
  [Amazon](Amazon)
- [Auto-Emoji](Auto-Emoji)
- [Archaeological databases](Archaeological_databases)
- [The Headless Bicycle](The_Headless_Bicycle)
- [Emo Face](Emo_Face)
- Character Locomotion Middleware - [Frontier](Frontier)
- Automatic accessibility assessor - [Frontier](Frontier)
- City generation - [Frontier](Frontier)

## Potential clients that did not proceed, but could be considered in future

- 2022: [Autodesk](Autodesk)
- 2021: [FetoLife](FetoLife), [Oodle](Oodle), [NHS
  Digital](NHS_Digital), [Jump
  Trading](Jump_Trading), [Microsoft Africa Research
  Institute](Microsoft_Africa_Research_Institute)
- 2020: [Argon Design](Argon_Design),
  [Gearset](Gearset), [Thales](Thales), [Sainsbury
  Laboratory](Sainsbury_Laboratory)
- 2019: [Smart Cambridge](Smart_Cambridge),
  [Mindi](Mindi), [Fauna and Flora
  International](Fauna_and_Flora_International),
  [Fusepump](Fusepump), [Investre](Investre),
- 2018: [Sparx](Sparx), [Cambridge
  Consultants](Cambridge_Consultants),
  [Capita](Capita), [Nominet
  Trust](Nominet_Trust), [University Information
  Services](University_Information_Services), [Sport
  England](Sport_England)
- 2017: [BT](BT), [Palantir](Palantir), [Elm
  Partners](Elm_Partners), [Lucid /
  Cycorp](Lucid_/_Cycorp), [Microsoft
  Research](Microsoft_Research)
- 2016: [Care Quality Commission](Care_Quality_Commission),
  [King Digital Entertainment](King_Digital_Entertainment),
  [The Fusion Works](The_Fusion_Works)
- 2015: [Thales](Thales), [Steve
  Wade](Steve_Wade), [Cambridge Humanitarian
  Centre](Cambridge_Humanitarian_Centre)
- 2014: [Broadcom](Broadcom), [last.fm](last.fm),
  [Rangespan](Rangespan), [John
  McMillan](John_McMillan), [Atheon](Atheon),
  [OpenMarket](OpenMarket), [Google](Google),
  [Repindex](Repindex)
- 2013: [Neul.com](Neul.com), [Steve Smith, CU Management
  Information Systems
  Division](Steve_Smith,_CU_Management_Information_Systems_Division),
  [Palantir Technologies](Palantir_Technologies), [Privacy
  International](Privacy_International)

## Archived records of previous years

=== "2026"
    [Design briefs](2026)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=y3ScrljJKGPnPOFt&amp;list=PLstyePOvf2d08_lFgNpoKEm1P7YHorYvJ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2025"
    [Design briefs](2025)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=y3ScrljJKGPnPOFt&amp;list=PLstyePOvf2d1HSBklMXWbdLd_4T2X8pIT" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2024"
    [Design briefs](2024) 
    
    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=xz9xVwFrww2yms8-&amp;list=PLstyePOvf2d3jOfc6j8cG7CmmVD6LyMe_" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2023"
    [Design briefs](2023) 
    
    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=Jkz7wVIPDfVFnuuZ&amp;list=PLstyePOvf2d2ZvC92BQkpR6WiaiROytev" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2022"
    [Design briefs](2022), [Final list of clients](2022_-_final_list_of_clients)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=Jkz7wVIPDfVFnuuZ&amp;list=PLstyePOvf2d2ZvC92BQkpR6WiaiROytev" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2021"
    [Design briefs](2021), [Final list of clients](2021_-_final_list_of_clients) and playlist:

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=j0wRt8EOCA9KMVTC&amp;list=PLstyePOvf2d2o9O_K3GEY3wAtbqkkYjPm" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2020"
    [Design briefs](2020), [Final list of clients](2020_-_final_list_of_clients), [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1920.pdf)

    (presentations disrupted by Covid-19 - no video).

=== "2019"
    [Design briefs](2019), [Final list of clients](2019_-_final_list_of_clients), [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1819.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=63bzsXlZvCeU2Yrf&amp;list=PLstyePOvf2d39LYQ3YBcdWUdREimYF8e4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2018"
    [Design briefs](2018), [Final list of clients](2018_-_final_list_of_clients), 
    [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1718.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=63bzsXlZvCeU2Yrf&amp;list=PLstyePOvf2d39LYQ3YBcdWUdREimYF8e4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2017"
    [Design briefs](2017), [Final list of clients](2017_-_final_list_of_clients), [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1617.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=rmRd86vSgXvRA_Vk&amp;list=PLstyePOvf2d0fbeximiXyWR2gB5KjjjE6" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2016"
    [Design briefs](2016), [Final list of clients](2016_-_final_list_of_clients), [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1516.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=P3Vy5jZsf8SaSDOe&amp;list=PLstyePOvf2d2QbiVFI4naska1n4G0fhSW" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2015"
    [Design briefs](2015), [Final list of clients](2015_-_final_list_of_clients), [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1415.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=P6xyZ2jBQzaoOuMO&amp;list=PLstyePOvf2d0cPplAB3DxHoaJC6hTYomP" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2014"
    [Design briefs](2014),
    [Final list of clients](2014_-_final_list_of_clients)
    [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1314.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=1joGQlbTBsY4EZGj&amp;list=PLstyePOvf2d2A3vWW4DkEGIAUIm4T0fg6" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

=== "2013"
    [Design briefs](2013),
    [Final list of clients](2013_-_final_list_of_clients),
    [Student briefing booklet](http://www.cl.cam.ac.uk/teaching/group-projects/StudentBriefing_1213.pdf)

    <iframe width="700" height="390" src="https://www.youtube.com/embed/videoseries?si=aGMf8DhF47FUpF7-&amp;list=PLstyePOvf2d2Dv0ISPIdNi3TdZW5QbJtf" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## Projects that have been offered, but not assigned to groups

The usual reason for cancelling a project is that the topic has not
attracted sufficient interest from students. It's worth keeping an eye
on these, as some topics, or ways of phrasing them, seem less
attractive. But fashions change!

- 2019: [Flyathlon (2019 version)](Flyathlon_(2019_version),
  [Robot Death Watch](Robot_Death_Watch), [Visual Pick and
  Place](Visual_Pick_and_Place)
- 2018: [Citizen Speed Safety](Citizen_Speed_Safety), [Every
  Car in Cambridge](Every_Car_in_Cambridge)
- 2017: [Energy with Social
  Conscience](Energy_with_Social_Conscience), [Science for
  AD2500](Science_for_AD2500), [Surgery in the
  Cloud](Surgery_in_the_Cloud)
- 2016: [Pocket Brain Surgeon](Pocket_Brain_Surgeon),
  [Reducing food waste with
  IoT](Reducing_food_waste_with_IoT), [The Busking
  Bus-Stop](The_Busking_Bus-Stop), [Safer Chicken from Farm
  to Fork](Safer_Chicken_from_Farm_to_Fork)
- 2015: [Reinfection Monitor](Reinfection_Monitor), [Online
  Identity for the Base of the
  Pyramid](Online_Identity_for_the_Base_of_the_Pyramid),
  [Audio Websites on
  Smartphones](Audio_Websites_on_Smartphones)
- 2014: [Locally Augmented Retail](Locally_Augmented_Retail),
  [Set Builder](Set_Builder), [Employability
  Coach](Employability_Coach), [Dance Practice
  Assistant](Dance_Practice_Assistant), [History
  Scraper](History_Scraper)
- 2013: [Touch screen prototyping at
  school](Touch_screen_prototyping_at_school), [Measuring
  glass-to-glass video-conference
  latency](Measuring_glass-to-glass_video-conference_latency),
  [The Poet Laureate's web
  thresholds](The_Poet_Laureate's_web_thresholds),
  [Countryside web server](Countryside_web_server), [Science
  exhibit interaction
  adviser](Science_exhibit_interaction_adviser), [Personal
  status server](Personal_status_server)
